...
export default async function approval(proposals, credentials) {
  logger.info("Starting Facta approval batch process", { 
  ...
  if (!loginResult.status) {
    logger.error("Failed to login to Facta approval system", { error: loginResult.data });
    await browser.close();
    return [];
  }
  ...
  logger.info("Facta approval batch process complete", { 
    proposalCount: proposals.length, 
