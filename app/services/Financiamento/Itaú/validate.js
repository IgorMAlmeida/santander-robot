export default function validate(data) {
  ...
  if (!data.phone || typeof data.phone !== 'string' || !phonePattern.test(data.phone)) {
    errors.phone = 'Telefone inválido';
  }

  if (data.property_type !== 'Residencial' && data.property_type !== 'Comercial') {
  ...
  if (
    ...
    if(data.property_type === 'Comercial') {
      if (data.input_value < min) {
        errors.input_value = `Valor de entrada deve ser no mínimo 50% do valor do imóvel para imóveis comerciais (${min.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})`;
      }
    }

    if(data.property_type === 'Residencial') {
      if (data.input_value < min || data.input_value > max) {
        errors.input_value = `Valor de entrada deve ser entre 10% e 90% do valor do imóvel para imóveis residenciais (${min.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} - ${max.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })})`;
      }
    }

  }

  if (!data.birth_date || typeof data.birth_date !== 'string' || !birthDatePattern.test(data.birth_date)) {
  ...
  if (
    ...
    errors.financing_term = data.property_type === 'Comercial' 
  ...
  if (data?.time_acquire_property && !validTimes.includes(data?.time_acquire_property)) {
