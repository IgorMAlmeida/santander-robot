...
export default async function searchUser(page, cpf) {
  try {
    logger.debug("Acessando tela de busca de usuarios");
    await sleep(1000);
    logger.debug("Antes do clique no autorizador");
    await clickElementByXpath(page, '//*[@id="block"]/a'); //Autorizador
    logger.debug("Após o clique no autorizador");

    await clickElementByXpath(page, '//*[@id="WFP2010_MPCDCOPUSUMST_I"]');//INCLUIR operador
